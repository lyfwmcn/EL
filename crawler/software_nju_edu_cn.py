"""
南京大学软件学院通知公告爬虫 (Software NJU Scraper)
爬取网站: https://software.nju.edu.cn
爬取内容: 通知公告 (页面内嵌JS数据)
存储方式: MySQL
...
TZGG_URL = f"{BASE_URL}/tzgg/index.html"
MAX_RECORDS = 400
REQUEST_TIMEOUT = 15
...
TABLE_NAME = "software_nju_edu_cn_news"
SCHEDULE_TIME = "08:00"
...
def scrape_software() -> list[dict]:
    """从页面内嵌JS数据中提取通知公告"""
    items = []

    try:
        resp = requests.get(TZGG_URL, headers=HEADERS, timeout=REQUEST_TIMEOUT)
    ...
    # 提取 dataList 中的完整 JSON 数组（贪婪匹配确保捕获完整结构）
    match = re.search(r'var dataList=(\[.*\]);', html, re.DOTALL)
    if not match:
        print("⚠️ 未找到 dataList")
        return items
    ...
    # data 格式: [{"infolist": [...15条]}, {"infolist": [...15条]}, ...]  共10组=150条
    for section in data:
...
def run_once():
    ...
    print(f"\n{'='*60}")
    print(f"🚀 软件学院爬虫启动 {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*60}")

    init_database()

    items = scrape_software()

    if not items:
        print("❌ 未爬取到任何数据")
        return

    print(f"📰 通知公告: {len(items)} 条")
    save_to_mysql(items)
...
def main():
    ...
    print(f"⏰ 软件学院定时模式启动，每天 {SCHEDULE_TIME} 执行")
