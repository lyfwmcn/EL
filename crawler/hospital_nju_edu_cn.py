"""
南京大学校医院通知公告爬虫 (Software NJU Scraper)
爬取网站: https://hospital.nju.edu.cn
爬取内容: 通知公告 (页面内嵌JS数据)
存储方式: MySQL
...
BASE_URL = "https://hospital.nju.edu.cn"
NEWS_PAGES = {
    "公告通知": "xwgg/ggtz/index.html",
}
...
TABLE_NAME = "hospital_nju_edu_cn_news"
SCHEDULE_TIME = "08:00"
...
def scrape_all() -> list[dict]:
    ...
    for category, path in NEWS_PAGES.items():
        url = f"{BASE_URL}/xwgg/ggtz/index.html"
        try:
...
def run_once():
    ...
    print(f"\n{'='*60}")
    print(f"🚀 校医院爬虫启动 {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*60}")
...
def main():
    ...
    print(f"⏰ 校医院定时模式启动，每天 {SCHEDULE_TIME} 执行")
